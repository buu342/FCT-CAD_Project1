...
__global__ void pointFilter(texel* out, texel* in, const uint32_t size, const float saturation, const float desaturation)
{
    // Calculate the texel coordinate that this thread will modify
...
int main(int argc, char* argv[]) 
{
    ...
    pointFilter<<<blockCount, threadCount>>>(d_out, d_out, imgsize, saturation, 1-saturation);
    cudaDeviceSynchronize();
