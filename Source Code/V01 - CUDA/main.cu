...
/*==============================
    areaFilter
    Performs a gaussian blur on a pixel of an image using CUDA
    @param The image array to output to
    @param The image array to read from
    @param The image size (in texels)
    @param The image width (in texels)
    @param The image height (in texels)
    @param The gaussian blur matrix
==============================*/

__global__ void areaFilter(texel* out, texel* img, int size, int width, int height, blurMatrix filter)
{
...
/*==============================
    pointFilter
    Desaturates a pixel on an image using CUDA
    @param The image array to output to
    @param The image array to read from
    @param The image size (in texels)
...
__global__ void pointFilter(texel *out, texel *img, size_t size, float saturation) 
{
    // Calculate the texel coordinate that this thread will modify
    size_t index = blockIdx.x*blockDim.x + threadIdx.x;
...
int main(int argc, char* argv[]) 
{
    ...
    pointFilter<<<blockCount, threadCount>>>(d_out, d_out, imgsize, saturation);
    cudaDeviceSynchronize();
