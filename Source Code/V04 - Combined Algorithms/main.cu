...
__global__ void shader(texel* out, texel* in, const uint32_t size, const uint32_t width, const uint32_t height, const blurMatrix filter, const float saturation, const float desaturation)
{
...
int main(int argc, char* argv[]) 
{
    ...
    shader<<<blockCount, threadCount>>>(d_out, d_in, imgsize, imgw, imgh, filter, saturation, 1-saturation);
